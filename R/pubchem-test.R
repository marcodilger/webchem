...
hazard_codes <- str_extract(hazard_statements, "^H\\d{3}") # needs to be improved to capture minimum classification and subclasses as well, see https://en.wikipedia.org/wiki/GHS_hazard_statements
hazard_codes <- sort(unique(hazard_codes[hazard_codes != ""]))



info <- list(source_name = source_name, substance = source_substancename, codes = hazard_codes, hazard_statements = hazard_statements, source_desc = source_description, url = source_url, license = source_license)
